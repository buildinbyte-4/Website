    ...
    <section id="services" className="border-b border-slate-200 bg-white py-20 dark:border-white/10 dark:bg-canvas">
      <div className="mx-auto max-w-7xl px-6">
        <div id="work" className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-3 font-display text-4xl font-semibold tracking-tight text-slate-950 sm:text-5xl dark:text-white">Engineering that fits the business.</h2>
        ...
        <div className="my-20 rounded-2xl border border-slate-200 bg-slate-50 p-6 dark:border-white/10 dark:bg-white/[0.03] sm:p-8">
          <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
            <div><p className="text-sm font-semibold text-brand-600 dark:text-brand-400">Our process</p><h3 className="mt-1 font-display text-2xl font-semibold text-slate-950 dark:text-white">A clear path from idea to launch</h3></div>
          ...
          <ol className="mt-8 grid gap-3 sm:grid-cols-4 lg:grid-cols-7">
            {process.map((label, index) => (
              <li key={label} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-white dark:bg-white">
        ...
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div><p className="text-sm font-semibold text-brand-600 dark:text-brand-400">Services</p><h3 className="mt-2 font-display text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl dark:text-white">What we can build together</h3></div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {services.map(({ icon: Icon, title, desc }) => (
            <article key={title} className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-md dark:border-white/10 dark:bg-bg-surface-dark dark:hover:border-brand-400/30">
              ...
              <h4 className="mt-5 font-display text-xl font-semibold text-slate-950 dark:text-white">{title}</h4>
