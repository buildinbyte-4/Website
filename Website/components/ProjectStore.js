...
import { FILTER_TABS } from '@/lib/data';

export default function ProjectStore({ customProjects, loading, onOpenDemo, onOpenInquiry }) {
  const [activeCategory, setActiveCategory] = useState('All');
  const [searchQuery, setSearchQuery] = useState('');

  const displayList = customProjects || [];
  ...
  const getMetrics = (id) => {
    ...
    // Hash the id (works for both numbers and UUID strings) to pick a variant
    const hash = String(id).split('').reduce((acc, ch) => acc + ch.charCodeAt(0), 0);
    if (hash % 3 === 0) return [{ label: 'Conv.', value: '+45%' }, { label: 'Speed', value: '0.8s' }, { label: 'API', value: '1M+' }];
    if (hash % 2 === 0) return [{ label: 'Sync', value: '<50ms' }, { label: 'Ret.', value: '+22%' }, { label: 'Up', value: '99.99%' }];
  ...
  return (
    <section id="projects" className="py-20 bg-brutal-bg border-b-4 border-brutal-black">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Project Card Grid */}
        {loading ? (
          <div className="text-center py-20 border-4 border-brutal-black bg-white shadow-brutal">
            <span className="font-black text-xl uppercase text-brutal-black">Loading solutions...</span>
          </div>
